...
import type { BookForm } from "./add-book";
...
const EditBook: React.FC<EditBookProps> = ({
  ...
  useEffect(() => {
    if (visible && initialBook) {
      form.setFieldsValue({
        ...initialBook,
        thumbnail:
          initialBook.images && initialBook.images[0]
            ? [
                {
                  uid: "-1",
                  name: "thumbnail.png",
                  status: "done",
                  url: initialBook.images[0],
                },
              ]
            : [],
        slider:
          initialBook.images && initialBook.images.length > 1
            ? initialBook.images.slice(1).map((img, index) => ({
                uid: index.toString(),
  ...
  const handleOk = async () => {
    try {
      ...
      // Tách fileList cho thumbnail và slider
    ...
    >
     <Divider />
      <Form form={form} layout="vertical" className="space-y-4">
        ...
        {/* Giá Cũ - Giá Mới - Số Lượng - Tác Giả */}
