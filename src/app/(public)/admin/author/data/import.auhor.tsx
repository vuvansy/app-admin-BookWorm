...
const ImportAuthor = (props: IProps) => {
    const { setOpenModalImport, openModalImport } = props;
    const templateFileUrl = "/template/author_bookworm.xlsx";
    ...
    const handleImport = async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/v1/author-many`, {
                method: "POST",
                headers: {
            ...
            if (!res.ok) {
                notification.error({
                    message: "Tạo tác giả hàng loạt không thành công",
            ...
            notification.success({
                message: "Tạo tác giả hàng loạt thành công",
                description: `Success = ${data?.data?.countSuccess || 0}. Error = ${data?.data?.countError || 0}`
            });
            props.onSuccess();
            setDataImport([]);
    ...
    return (
        <>
            <Modal title="Import data author"
                width={"50vw"}
